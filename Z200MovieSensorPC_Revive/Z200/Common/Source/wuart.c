...
/*
				for(y=0;y<=u16RxSize+2;y++){
					DBG_vPrintf(1, "pu8RxPayload[%d]=%02x\r\n", y,pu8RxPayload[y]);
				}
*/

				/*
				if(pu8RxPayload[10]==0xff){

					if((memcmp(&pu8RxPayload[9],&pu8RxPayload[11],2))!=0){
						memcpy(OutData,HRhead,2);

						for(x=10;x<22;x++){
							Checksum ^=pu8RxPayload[x];
						}
						//DBG_vPrintf(1, "Checksum=%02x\r\n",Checksum);
						if(Checksum==pu8RxPayload[22]){
						*/
				            memcpy(OutData,HRhead,2);
...
*/
			}else if(pu8RxPayload[0] == 0x03){    //HRsensor Reper
				...
				/*
				memcpy(&HROutData[HROutData_Head].Data,HRhead,2);
				HROutData[HROutData_Head].Data[3]= pu8RxPayload[4]; //source addresss LO
				HROutData[HROutData_Head].Data[4]= pu8RxPayload[5]; //source addresss HI
				HROutData[HROutData_Head].Data[5]= pu8RxPayload[2]; //Dest addresss LO
				HROutData[HROutData_Head].Data[6]= pu8RxPayload[3]; //Dest addresss HI
				HROutData[HROutData_Head].Data_Head=7;
				for (u16Pos = 8; u16Pos < u16RxSize; u16Pos++)
				{
				...
				HROutData[HROutData_Head].Data[2]=u16RxSize-4;  //len
