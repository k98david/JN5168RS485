...
void timer_tick_Callback(uint32 u32Device, uint32 u32ItemBitmap);
/****************************************************************************/
/***        Exported Variables                                            ***/
/****************************************************************************/
...
//	WUART_vInit();
	...
	//if (u32AHI_DioReadInput() & (1<< KEY0pin)){
		...
		gConfigData.settings.stopBit=E_AHI_UART_1_STOP_BIT;
		gConfigData.settings.deviceType=dtSlave; //Master
	...
	vAHI_TickTimerRegisterCallback(timer_tick_Callback);
    ...
    vAHI_TickTimerWrite(1);
    vAHI_TickTimerInit(timer_tick_Callback);
    vAHI_TickTimerInterval(8 * TICK_TIMER_MS);
    vAHI_TickTimerConfigure(E_AHI_TICK_TIMER_RESTART);
    vAHI_TickTimerIntEnable(TRUE);
*/
...
PUBLIC void timer_tick_Callback(uint32 u32Device, uint32 u32ItemBitmap){

	DBG_vPrintf(1, "\r\n%d timer_tick_Callback()");
