...
SensorCMDTable_t SensorCMD_Table;

SensorEECMDTable_t SensorEECMD_Table[SensorCMDNumber];
...
extern bool Timer4_Watchdog_flag;

/*Fifo Buffer */
FifoBuffer_t Fifo_Data[30]={0};
uint8_t FifoHead=0,FifoTail=0,CMD_Table_Index=0;
...
uint8 processSCMD(char * cmdStr){
    ...
    for(x=0;x<=Len+5;x+=2){
    	SensorEECMD_Table[EEGroup].sensorCmdTable_Num[EEArrGroup].sensorCmd[y++]=convertASCII2Hex(Temp+x);
    }

    SensorEECMD_Table[EEGroup].sensorCmdTable_Num[EEArrGroup].sensorCmdLen=Len;

    DBG_vPrintf(Debug, "SensorEECMD_Table.[EEGroup].Len=%d\r\n", SensorEECMD_Table[EEGroup].sensorCmdTable_Num[EEArrGroup].sensorCmdLen);

	DBG_vPrintf(Debug, "SensorEECMD_Table CMD\r\n");
...
/* old no eeprome add code*/
	...
	DBG_vPrintf(Debug, "Len=%d CMD_Number=%d\r\n", Len,CMD_Number);
    for(x=0;x<=Len+5;x+=2){
    	SensorCMD_Table.sensorCmdTable_Num[CMD_Number].sensorCmd[y++]=convertASCII2Hex(Temp+x);
    }

    SensorCMD_Table.sensorCmdTable_Num[CMD_Number].sensorCmdLen=Len;

    DBG_vPrintf(Debug, "SensorCMD_Table Len=%d\r\n", SensorCMD_Table.sensorCmdTable_Num[CMD_Number].sensorCmdLen);

	DBG_vPrintf(Debug, "sensorList CMD\r\n");
	for(y=0;y<SensorCMD_Table.sensorCmdTable_Num[CMD_Number].sensorCmdLen;y++){
		DBG_vPrintf(Debug, "%x", SensorCMD_Table.sensorCmdTable_Num[CMD_Number].sensorCmd[y]);
	}

	totalSegment = u16AHI_InitialiseEEP(&EEPROMDataLength);
	iAHI_EraseEEPROMsegment(CMD_Number+EEpromCMDTable+EEpromAddress);
	iAHI_WriteDataIntoEEPROMsegment(CMD_Number+EEpromCMDTable+EEpromAddress,0,&SensorCMD_Table.sensorCmdTable_Num[CMD_Number],64);
	PSW_CMD_flag=FALSE;  //restart RS485 Loop
*/
...
uint8 processRfLoopTime(char * cmdStr){
	...
	DBG_vPrintf(Debug, "Index=%d\r\n",Index);

    ASCII=(cmdStr[2]<<8)+cmdStr[3];
    LoopTemp=convertASCII2Hex(&ASCII);

	DBG_vPrintf(Debug, "Index=%d LoopTemp=%d\r\n",Index, LoopTemp);
...
uint8 processCMD1(char * cmdStr){
	DBG_vPrintf(Debug, "evRfLsCMD1_Event\r\n");
	sendEvent(evRfLsCMD1_Event);
	return 0;
}

uint8 processCMD2(char * cmdStr){
	DBG_vPrintf(Debug, "evRfLsCMD2_Event\r\n");
...
*/
	for(x=0;x<20;x++){
		...
		if(sensorList[x].info.sensorAddress!=0){
			...
			DBG_vPrintf(Timer_Debug, "sensorList[%d].Address=%x y=%d\r\n",x, sensorList[x].info.sensorAddress,y);
			sensorList[x].info.sensorType=cmdStr[y+1];
			DBG_vPrintf(Timer_Debug, "sensorList[%d].Type=%x y=%d\r\n",x, sensorList[x].info.sensorType,y);
			sensorList[x].info.CMD_Table_number[0]=cmdStr[y+2];
			DBG_vPrintf(Timer_Debug, "sensorList[%d].CMD NUM[0]=%d y=%d\r\n",x, sensorList[x].info.CMD_Table_number[0],y);

			// Count Time Uint16T
			DBG_vPrintf(1, "cmdStr[y+3]=%02x\r\n",cmdStr[y+3]);
			DBG_vPrintf(1, "cmdStr[y+4]=%02x\r\n",cmdStr[y+4]);

			sensorList[x].info.LoopTime=(cmdStr[y+3]<<8)+(cmdStr[y+4]&0xff);
			DBG_vPrintf(Timer_Debug, "sensorList[%d].LoopTime=%d y=%d\r\n",x, sensorList[x].info.LoopTime,y);
			/*
	...
	*/
			...
			sensorList[x].info.Enable=0;
			sensorList[x].info.mask=0;
			sensorList[x].info.CmdCount=0;
			sensorList[x].info.LoopTime=0;
			sensorList[x].info.CountTime=0;


			DBG_vPrintf(Timer_Debug, "sensorList[%d].Address=%x y=%d\r\n",x, sensorList[x].info.sensorAddress,y);
			sensorList[x].info.sensorType=0;
			DBG_vPrintf(Timer_Debug, "sensorList[%d].Type=%x y=%d\r\n",x, sensorList[x].info.sensorType,y);
			sensorList[x].info.CMD_Table_number[0]=0;
			DBG_vPrintf(Timer_Debug, "sensorList[%d].CMD NUM[0]=%d y=%d\r\n",x, sensorList[x].info.CMD_Table_number[0],y);
...
uint8 processXCMD(char * cmdStr){
	...
	for(y=0;y<30;y++){
		DBG_vPrintf(Timer_Debug, "cmdStr[%d]=%02x\r\n",y,cmdStr[y]);
	}
    y=0;
	...
	for(x=0;x<10;x++){
		Len=cmdStr[y];
		if((Len!=0)&&(Len<=15)){
			SensorCMD_Table.sensorCmdTable_Num[x].sensorCmdLen=Len;
			memcpy(SensorCMD_Table.sensorCmdTable_Num[x].sensorCmd,&cmdStr[y+1],Len);

			y=Len+y+1;

		    DBG_vPrintf(Timer_Debug, "SensorCMD_Table Len=%d\r\n", SensorCMD_Table.sensorCmdTable_Num[x].sensorCmdLen);

			//DBG_vPrintf(1, "sensorList CMD\r\n");
			for(z=0;z<SensorCMD_Table.sensorCmdTable_Num[x].sensorCmdLen;z++){
				DBG_vPrintf(Timer_Debug, "sensorList CMD[%d]=%02x\r\n",z, SensorCMD_Table.sensorCmdTable_Num[x].sensorCmd[z]);
			}

			totalSegment = u16AHI_InitialiseEEP(&EEPROMDataLength);
			iAHI_EraseEEPROMsegment(x+EEpromCMDTable+EEpromAddress);
			iAHI_WriteDataIntoEEPROMsegment(x+EEpromCMDTable+EEpromAddress,0,&SensorCMD_Table.sensorCmdTable_Num[x],64);
		}else{
			SensorCMD_Table.sensorCmdTable_Num[x].sensorCmdLen=0;
			SensorCMD_Table.sensorCmdTable_Num[x].sensorCmd[0]=0;
			totalSegment = u16AHI_InitialiseEEP(&EEPROMDataLength);
			iAHI_EraseEEPROMsegment(x+EEpromCMDTable+EEpromAddress);
			iAHI_WriteDataIntoEEPROMsegment(x+EEpromCMDTable+EEpromAddress,0,&SensorCMD_Table.sensorCmdTable_Num[x],64);
			y++;
...
uint8 processCMDR(char * cmdStr){
    ...
    ASCII=(cmdStr[0]<<8)+cmdStr[1];
    CMDR_Index=convertASCII2Hex(&ASCII);
	DBG_vPrintf(Debug, "CMDR_Index=%02x\r\n",CMDR_Index);

	sendEvent(evCMDR_Event);
	return 0;
}
...
uint8_t check_HaveData(SensorLog_t *sensorTemp){
	...
	for(i=0;i<MaxSensorNumber;i++){
		DBG_vPrintf(Debug, "sensorList Type%x\r\n", sensorList[i].info.sensorType);
		DBG_vPrintf(Debug, "sensorList Enable%d\r\n", sensorList[i].info.Enable);
...
PUBLIC void timer4_Polling_Callback(uint32 u32Device, uint32 u32ItemBitmap){
	...
	if(timeCount==2){
		for(x=0;x<MaxSensorNumber;x++){    //full loop
			if((sensorList[x].info.Enable!=0)&&(sensorList[x].info.CmdCount<=sensorList[x].info.LoopTime)&&(sensorList[x].info.sensorAddress!=0)){
				sensorList[x].info.CountTime++;
				//DBG_vPrintf(1, "sensorList[%d].info.CountTime=%02d\r\n",x,sensorList[x].info.CountTime);
			}
		}
		DBG_vPrintf(Timer_Debug, "sensorList[%d].info.CountTime=%02d\r\n",0,sensorList[0].info.CountTime);
		timeCount=0;
...
uint8_t getSensorCMD(uint8_t *sData){
	...
	if ((sensorList[gCurrentSensorCMDIndex].info.Enable!=0)){
		DBG_vPrintf(Debug, "gCurrentSensorCMDIndex %x\r\n", gCurrentSensorCMDIndex);
		...
		//if((sensorList[gCurrentSensorCMDIndex].info.CmdCount>0) && (gTXRS485Sensor_Count<(sensorList[gCurrentSensorCMDIndex].info.CmdCount)-1)){
			DBG_vPrintf(Debug, "gTXRS485Sensor_Count T %x\r\n", gTXRS485Sensor_Count);
			gRXRS485Sensor_Count=gTXRS485Sensor_Count;

			memcpy(Temp,SensorCMD_Table.sensorCmdTable_Num[sensorList[gCurrentSensorCMDIndex].info.CMD_Table_number[gTXRS485Sensor_Count++]].sensorCmd,SensorCMD_Table.sensorCmdTable_Num[sensorList[gCurrentSensorCMDIndex].info.CMD_Table_number[gTXRS485Sensor_Count]].sensorCmdLen);
		    Len=strlen(Temp);

			DBG_vPrintf(Debug, "Len=%d gCurrentSensorCMDIndex=%d\r\n", Len,gCurrentSensorCMDIndex);
		    calc_crc(Temp,Len);
		    for(x=0;x<=Len;x+=2){
				DBG_vPrintf(Debug, "Temp[%d]=%02x\r\n",x ,Temp[x]);
		    }
			...
			memcpy(&Temp[1],SensorCMD_Table.sensorCmdTable_Num[sensorList[gCurrentSensorCMDIndex].info.CMD_Table_number[gTXRS485Sensor_Count]].sensorCmd,SensorCMD_Table.sensorCmdTable_Num[sensorList[gCurrentSensorCMDIndex].info.CMD_Table_number[gTXRS485Sensor_Count]].sensorCmdLen);


			Len=SensorCMD_Table.sensorCmdTable_Num[sensorList[gCurrentSensorCMDIndex].info.CMD_Table_number[gTXRS485Sensor_Count]].sensorCmdLen;
			Len=Len+1;
			DBG_vPrintf(Debug, "Len=%d gCurrentSensorCMDIndex=%d\r\n", Len,gCurrentSensorCMDIndex);
		    calc_crc(Temp,Len);
		    for(x=0;x<Len+2;x++){
		    ...
		    for(x=0;x<Len+2;x++){
				DBG_vPrintf(Debug, "sData[%d]=%02x\r\n",x ,sData[x]);
		    }

			DBG_vPrintf(Debug, "gTXRS485Sensor_Count F %x\r\n", gTXRS485Sensor_Count);
...
void readEEpromSensor(void){
	...
	for(x=0;x<10;x++){
		...
		for(y=0;y<4;y++){
		    DBG_vPrintf(1, "SensorEECMD_Table[%d].sensorCmdTable_Num[%d].sensorCmdLen=%d\r\n",x,y, SensorEECMD_Table[x].sensorCmdTable_Num[y].sensorCmdLen);
			for(z=0;z<15;z++){
	...
	for(x=0;x<10;x++){
		...
		DBG_vPrintf(Timer_Debug, "SensorCMDTest_Table %d\r\n", x);
	    DBG_vPrintf(Timer_Debug, "SensorCMD_Table Len=%d\r\n", SensorCMD_Table.sensorCmdTable_Num[x].sensorCmdLen);

		DBG_vPrintf(Timer_Debug, "SensorCMDTest_Table CMD\r\n");

		for(y=0;y<SensorCMD_Table.sensorCmdTable_Num[x].sensorCmdLen;y++){
			DBG_vPrintf(Timer_Debug, "SensorCMDTest_Table.sensorCmdTable_Num[%d].sensorCmd[%d]=%x \r\n",x,y ,SensorCMD_Table.sensorCmdTable_Num[x].sensorCmd[y]);
		}
	}
*/
	for(x=0;x<MaxSensorNumber;x++){
		...
		DBG_vPrintf(Timer_Debug, "readEEpromSensor %d\r\n", x);

		DBG_vPrintf(Timer_Debug, "sensorList Count%x\r\n", sensorList[x].info.CmdCount);
		DBG_vPrintf(Timer_Debug, "sensorList mask%x\r\n", sensorList[x].info.mask);
		DBG_vPrintf(Timer_Debug, "sensorList Type%x\r\n", sensorList[x].info.sensorType);
		DBG_vPrintf(Timer_Debug, "sensorList Enable%d\r\n", sensorList[x].info.Enable);
		DBG_vPrintf(Timer_Debug, "sensorList Address%x\r\n", sensorList[x].info.sensorAddress);
		DBG_vPrintf(Timer_Debug, "sensorList LoopTemp %d\r\n", sensorList[x].info.LoopTime);
		DBG_vPrintf(Timer_Debug, "sensorList CountTime %d\r\n", sensorList[x].info.CountTime);

		DBG_vPrintf(Timer_Debug, "sensorList CMD NUM[0] %d\r\n", sensorList[x].info.CMD_Table_number[0]);
	}
}
...
void CpydataFifo(uint8_t Number){
	uint8_t z=0,y=0,x=0;
    uint8_t Temp[50];
    uint8 EEGroup=0,EEArrGroup=0;

	for(x=0;x<10;x++){
		iAHI_ReadDataFromEEPROMsegment(x+EEpromCMDTable+EEpromAddress,0,&SensorEECMD_Table[x],64);
	}

	EEGroup=(sensorList[Number].info.CMD_Table_number[0]/4);
	EEArrGroup=(sensorList[Number].info.CMD_Table_number[0]%4);

	sensorList[Number].info.CountTime=0;   //clean TimeCount

	Temp[0]=sensorList[Number].info.sensorAddress;

	memcpy(&Temp[1],SensorEECMD_Table[EEGroup].sensorCmdTable_Num[EEArrGroup].sensorCmd,SensorEECMD_Table[EEGroup].sensorCmdTable_Num[EEGroup].sensorCmdLen);


	Len=SensorEECMD_Table[EEGroup].sensorCmdTable_Num[EEArrGroup].sensorCmdLen;
	Len=Len+1;
	DBG_vPrintf(Debug, "Len=%d Number=%d\r\n", Len,Number);
	calc_crc(Temp,Len);
	...
	Fifo_Data[FifoHead].Index=sensorList[Number].info.CMD_Table_number[0];
	...
	DBG_vPrintf(Timer_Debug, "FifoHead=%d\r\n",FifoHead);
    ...
    /*  old code no eeprom */
    /*
	iAHI_ReadDataFromEEPROMsegment(Number+EEpromCMDTable+EEpromAddress,0,&SensorCMD_Table.sensorCmdTable_Num[Number],64);

	sensorList[Number].info.CountTime=0;   //clean TimeCount

	Temp[0]=sensorList[Number].info.sensorAddress;

	memcpy(&Temp[1],SensorCMD_Table.sensorCmdTable_Num[sensorList[Number].info.CMD_Table_number[0]].sensorCmd,SensorCMD_Table.sensorCmdTable_Num[sensorList[Test].info.CMD_Table_number[0]].sensorCmdLen);

	Len=SensorCMD_Table.sensorCmdTable_Num[sensorList[Number].info.CMD_Table_number[0]].sensorCmdLen;
	Len=Len+1;
	DBG_vPrintf(Debug, "Len=%d Number=%d\r\n", Len,Number);
	calc_crc(Temp,Len);
