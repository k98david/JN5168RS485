...
PRIVATE bool_t   Calculation_bNum2String		(uint8, uint32, uint8, uint8, char, bool_t, char, bool_t);
...
extern uint8 DecAddressTemp[10];
/****************************************************************************/
/***        Local Constants                                               ***/
/****************************************************************************/
...
PUBLIC 	bool_t Calulation_bPrintf_ap (
		uint8 			u8Uart,		/**< Uart to output to */
		const char     *pcFormat,	/**< Formatting string */
		va_list         ap)
{
	...
	if (u8Uart < 2 && asUart[u8Uart].bOpened)
	{
		...
		while ((c = *bp++)) {
			...
			/* %d - show a decimal value */
			case 'd':
				Calculation_bNum2String(u8Uart, va_arg(ap, uint32), 10, u8Min, cPad, bLeft, 0, bPrefix);
				break;
...
PRIVATE bool_t Calculation_bNum2String(uint8 u8Uart, uint32 u32Number, uint8 u8Base, uint8 u8Min, char cPad, bool_t bLeft, char cSign, bool_t bPrefix)
{
	uint8 x=0;
	/* Uart has been opened ? */
	if (u8Uart < 2 && asUart[u8Uart].bOpened)
