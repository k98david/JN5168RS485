...
HRsensor_sData HROutData[10];
uint8 HROutData_Head=0;
uint8 HROutData_Tail=0;
uint8 HRCount=0;
/****************************************************************************/
/***        Exported Variables                                            ***/
/****************************************************************************/
...
/* Command lengths */
uint8 			    u8CmdData      	= WUART_DATA_DATA;
...
uint8 OutData_Head=0;
uint8 OutData_Tail=0;
/****************************************************************************/
/***        Exported Functions                                            ***/
/****************************************************************************/
...
PUBLIC void WUART_vTick(void)
{
	...
	if (gDoApplicationResend ){
		DBG_vPrintf(WUART_TRACE, "\r\n%d WUART< Resend()", NODE_sData.u32Timer);
		if (gConfigData.settings.targetAddr == 0xFFFF){
			...
			DBG_vPrintf(WUART_TRACE, "\r\n%d WUART< target= %#x", NODE_sData.u32Timer, targetAddr);
		...
		if(HROutData[HRCount].OutFlag==TRUE){
			...
			while(HROutData[HRCount].Data_Tail != HROutData[HRCount].Data_Head){
				au8CmdData[u8CmdData++] = HROutData[HRCount].Data[HROutData[HRCount].Data_Tail++];
				if(HROutData[HRCount].Data_Tail>=45){
					HROutData[HRCount].Data_Tail=0;
				}
			}
			...
			HROutData[HRCount].OutFlag=FALSE;
			HRCount++;
			if(HRCount>10){
				HRCount=0;
			}
		...
		/*
		if(OutData_Tail != OutData_Head){   //Relay mode
			u8CmdData 	=  8; //The first 8 data is Network layer Header
			while(OutData_Tail != OutData_Head){
				au8CmdData[u8CmdData++] = OutData[OutData_Tail++];
				if(OutData_Tail>=NODE_PAYLOAD){
					OutData_Tail=0;
				}
			}
			au8CmdData[SEQ_POS] = WUART_u8MsgSeqTxNew();
		...
		if (WUART_sData.u64Online  != u64RxAddr || WUART_sData.u8MsgSeqRx != pu8RxPayload[7])
		{
			...
			if (pu8RxPayload[0] == 0x00){ //this is a data packet. Indicated by the first byte of Network Layer header.
				...
				*/
				...
				HROutData[HROutData_Head].SourceSensorAddress=pu8RxPayload[4]+(pu8RxPayload[5]<<8);   //source sensor Address
				au8CmdData[SRC_POS]=HROutData[HROutData_Head].SourceSensorAddress;
				au8CmdData[SRC_POS+1]=HROutData[HROutData_Head].SourceSensorAddress>>8;
				for (u16Pos = 8; u16Pos < u16RxSize; u16Pos++)
				{
					HROutData[HROutData_Head].Data[HROutData[HROutData_Head].Data_Head++]=pu8RxPayload[u16Pos];
					if(HROutData[HROutData_Head].Data_Head>=45){
						HROutData[HROutData_Head].Data_Head=0;
					}
				}
				HROutData[HROutData_Head].OutFlag=TRUE;  //flag
				HROutData_Head++;
				if(HROutData_Head>=10){
					HROutData_Head=0;
				}
				/*
				for (u16Pos = 0; u16Pos < u16RxSize; u16Pos++)
				{
					OutData[OutData_Head++]=pu8RxPayload[u16Pos];
					if(OutData_Head>=NODE_PAYLOAD){
						OutData_Head=0;
					}
				}
				*/
