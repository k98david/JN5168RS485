...
uint8 DecAddressTemp[10]={0};
bool PowerUpData=FALSE;
bool ADV_Pack=FALSE;
bool Timer4_Watchdog_flag=TRUE;
bool Timer0_Watchdog_flag=TRUE;
//uint8 L=0;
/****************************************************************************/
/***        Exported Functions                                            ***/
/****************************************************************************/
...
PRIVATE void CRD_vMain(void)
{
    ...
    if (gConfigData.settings.deviceType == dtMaster){
    	//printMsg("ATX=%d@Master V250\r\n",14);
    	//L=14;
		wuart_vPrintf(1, "ATX=%02x@Master V250\r\n",12);
    	//printMsg(Z200_Version);
    	CRD_vReqStartCoordinator();
    	if (gConfigData.settings.verbose){
			//wuart_vPrintf(1, "ATX=Starting as PAN coordinator on channel 0x%x.\r\n",	NODE_sData.u8Channel);
    	...
    	while (1)
		{
			if(Timer4_Watchdog_flag && Timer0_Watchdog_flag){
				vAHI_WatchdogRestart();
				Timer4_Watchdog_flag=FALSE;
				Timer0_Watchdog_flag=FALSE;
			}
			do
			{
    	...
    	//printMsg("ATX=%d@One2One Master V250\r\n",22);
    	//L=22;
		//wuart_vPrintf(1, "ATX=%02x@One2One Master V250\r\n",20);
    	//printMsg(Z200_Version);
		CRD_vReqStartCoordinator();
    	if (gConfigData.settings.verbose){
			//wuart_vPrintf(1, "ATX=Starting as One2One master on channel 0x%x.\r\n",	NODE_sData.u8Channel);
			//wuart_vPrintf(1, "ATX=Started with PAN ID %#x and short address %#x.\r\n", NODE_sData.u16PanId,	NODE_sData.u16Address);
			//printMsg("Ready to send and receive data.\r\n");
    	}
    	vAHI_WatchdogStart(8);   //1's
...
/*
    		...
    		//ED_vReqActiveScan();//Postponse started message until association response received.
        	if (gConfigData.settings.verbose){
        		wuart_vPrintf(1, "ATX=Starting as unpaired One2One slave.\r\nPlease do pairing.\r\n");
    			wuart_vPrintf(1, "ATX=Started with PAN ID %#x and short address %#x.\r\n", NODE_sData.u16PanId,	NODE_sData.u16Address);
        	}
    	}
		//wuart_vPrintf(1, "Started with PAN ID %#x and short address %#x.\r\n", NODE_sData.u16PanId,	NODE_sData.u16Address);
...
PRIVATE void CRD_vInit(void)
{
	...
	//if (u32AHI_DioReadInput() & (1<< KEY0pin)){
		...
		//gConfigData.settings.deviceType=dtOne2OneMaster; //Master
		//gConfigData.settings.deviceType=dtOne2OneSlave; //dtOne2OneSlave
    ...
    //iAHI_ReadDataFromEEPROMsegment(EEpromAddress,0,&sensorList[0].data,64);
	#endif
	...
	readEEpromSensor();    //Read EE Data Sensor list
	...
	if (!(u32AHI_DioReadInput() & (1<< KEY0pin))){
		...
		vAHI_Timer1RegisterCallback(Restorekey0_timer1Callback);
		vAHI_TimerStartRepeat(E_AHI_TIMER_1, 0,125);
	}else{
	...
	/* Start timer 3 for Key  */
	vAHI_TimerClockSelect(E_AHI_TIMER_3, FALSE, FALSE);
	vAHI_TimerSetLocation(E_AHI_TIMER_3,TRUE,FALSE);  //change Timer DIO to DIO5-8 if no this line DE RE cant use
	vAHI_TimerEnable(E_AHI_TIMER_3, 16,FALSE, TRUE,FALSE);    //409ms
	//vAHI_TimerStartRepeat(E_AHI_TIMER_3, 0,125);   //0.5 's
	vAHI_Timer3RegisterCallback(Key_timer3Callback);


	/* Start timer 2 for Polling  */
	//vAHI_TimerClockSelect(E_AHI_TIMER_4, FALSE, FALSE);
	//vAHI_TimerSetLocation(E_AHI_TIMER_4,TRUE,FALSE);  //change Timer DIO to DIO5-8 if no this line DE RE cant use
	//vAHI_TimerEnable(E_AHI_TIMER_4, 16,FALSE, TRUE,FALSE);    //409ms
	//vAHI_Timer4RegisterCallback(timer4_Polling_Callback);
...
PRIVATE void CRD_vIndData(MAC_McpsDcfmInd_s *psMcpsInd)
{
    ...
    uint64 u64SrcAddr;
	/* Debug */
	DBG_vPrintf(CRD_TRACE, "\r\n%d CRD < CRD_vIndData() Mode=%d Short=%#x TargetAddr=%x",
		NODE_sData.u32Timer, psFrame->sSrcAddr.u8AddrMode, psFrame->sSrcAddr.uAddr.u16Short,gConfigData.settings.targetAddr);
	...
	if (psFrame->sSrcAddr.u8AddrMode == 2)
	{
		...
		if(gConfigData.settings.deviceType == dtOne2OneSlave){
			if(psFrame->sDstAddr.uAddr.u16Short!=0xffff){
				ADV_Pack=FALSE;   //Not ADV Data
				if((gConfigData.settings.targetAddr)==(psFrame->sSrcAddr.uAddr.u16Short)){
...
PRIVATE void CRD_vIndAssociate(MAC_MlmeDcfmInd_s *psMlmeInd)
{
    uint16 u16ShortAddr = 0xffff,u16ShortAddrL=0xffff,U16Temp=0;
    uint16 u16EndDeviceIndex;
    uint8 addTempOut[20]={"@     joined.\r\n"};
    uint8 len=0,x=0;
	uint8 STM32[4]={"ATX="};
...
*/
    ...
    u16ShortAddr = (uint16)((psMlmeInd->uParam.sIndAssociate.sDeviceAddr.u32H >> 16));
    U16Temp=u16ShortAddr>>8;
    u16ShortAddr=u16ShortAddr-(u16ShortAddr&0xff00);
    u16ShortAddr=u16ShortAddr<<8;
    u16ShortAddr=u16ShortAddr+U16Temp;
	gWorkingMode = wmNormalMode_c;     //paring succeed

	u16ShortAddrL = (uint16)(psMlmeInd->uParam.sIndAssociate.sDeviceAddr.u32L & 0xffff);
	if (gConfigData.settings.verbose){
		//wuart_vPrintf(1, "@%d joined.\r\n",u16ShortAddr);
		//wuart_vPrintf(1, "@%d Ljoined.\r\n",u16ShortAddrL);
		wuart_vPrintf(1, "ATX=%02xATMac#%x\r\n",10,u16ShortAddrL);
		LED1On

/*
		u8CmdData 	=  8; //The first 8 data is Network layer Header
		au8CmdData[MAC_POS]=3;  //not Data Packet
		au8CmdData[TARGET_POS]=gConfigData.settings.targetAddr;
		au8CmdData[TARGET_POS+1]=gConfigData.settings.targetAddr>>8;
		//au8CmdData[6]=1;  //for AddRep Data to master

    	au8CmdData[SRC_POS]=u16ShortAddr;
		au8CmdData[SRC_POS+1]=u16ShortAddr>>8;

		au8CmdData[SEQ_POS] =0;
		au8CmdData[u8CmdData++]=2;  //for AddRep Data to master

		Calculation_vPrintf(1, "%d",u16ShortAddr);   //Calculayion Dec Address to DecAddressTemp
		//pch = strtok(addTempOut,0);
        len=strlen(addTempOut);
        memcpy(&addTempOut[1],&DecAddressTemp[1],DecAddressTemp[0]);

        for(x=0;x<len;x++){
    		au8CmdData[u8CmdData++]=addTempOut[x];  //for AddRep Data to master
        }
		if (gConfigData.settings.targetAddr == 0xFFFF){
		//if (0){
...
/*
		...
		OutData[5]= u16ShortAddrL; //source addresss LO
		OutData[6]= u16ShortAddrL>>8; //source addresss HI
		//OutData[10]= pu8RxPayload[2]; //Dest addresss LO
		//OutData[11]= pu8RxPayload[3]; //Dest addresss HI
		OutData[4]=4;  //len
		...
		*/
		...
		{
			OutData[u16Pos+4]=pu8RxPayload[u16Pos];
		}
		OutData[u16Pos+4]=0x0D;
		OutData[u16Pos+5]=0x0A;
		//OutData[12]=0x01;
		for (u16Pos = 0; u16Pos < u16RxSize+6; u16Pos++)
		{
			UART_bPutChar(WUART_sData.u8Uart, OutData[u16Pos]);
		}
		*/
